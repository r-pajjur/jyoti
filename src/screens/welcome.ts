...
export function welcomeScreen(): HTMLElement {
  ...
  const instructions = ios
    ? `
      <ol class="steps">
        ...
        <li>Close Safari, then open <strong>Dhara</strong> from your home screen — the little drop.</li>
      </ol>
      ${
        inSafari
          ? ''
    ...
    : `
      <ol class="steps">
        ...
        <li>Open <strong>Dhara</strong> from your home screen.</li>
      </ol>`;
  ...
  const node = el(`
    <section>
      <div class="center">
        ...
        <h1>Thirty mornings of drops</h1>
      ...
      <hr class="rule">
      <div class="card">
        <h2>First, put Dhara on your home screen</h2>
        <p class="muted small">It takes about twenty seconds, and it makes Dhara open like an app instead of a web page.</p>
        ${instructions}
      </div>
  ...
  if (isStandalone()) {
    node.querySelector('.card')?.replaceChildren(
      el('<div><h2>You are all set</h2><p class="muted">Dhara is installed. Let\'s get your name.</p></div>'),
    );
  }
  return node;
}
